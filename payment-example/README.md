...
1. This sample uses Square .NET SDK version **42.0.1**. The latest SDK can be found [here](https://developer.squareup.com/docs/sdks/dotnet).

1. Ensure you have `dotnet` installed (`dotnet --version` in your terminal). If not, please download [.NET 9.0+](https://dotnet.microsoft.com/download)
...
public ProcessPaymentModel(Microsoft.Extensions.Configuration.IConfiguration configuration)
{
    ...
    client = new SquareClient(accessToken, new ClientOptions { 
        BaseUrl = environment == SquareEnvironment.Sandbox ? 
...
public async Task<IActionResult> OnPostAsync()
{
    ...
    var retrieveLocationResponse = await client.Locations.GetAsync(
        new GetLocationsRequest { LocationId = locationId });
    ...
    catch (SquareApiException e)  // Updated exception type
    {
